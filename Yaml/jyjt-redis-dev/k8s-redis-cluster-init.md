启动一个centos来干活
```
kubectl run -it centos --image=centos:7 --restart=Never -n app-dev bash
```
创建cluster
```
# redis-cli --cluster create \
  `dig +short redis-0.redis.app-dev.svc.cluster.local`:6379 \
  `dig +short redis-1.redis.app-dev.svc.cluster.local`:6379 \
  `dig +short redis-2.redis.app-dev.svc.cluster.local`:6379
```
查找cluster id 
```
redis-cli -h `dig +short redis-0.redis.app-dev.svc.cluster.local` -p 6379
...
```
redis-cli  --cluster add-node `dig +short redis-3.redis.app-dev.svc.cluster.local`:6379 `dig +short redis-0.redis.app-dev.svc.cluster.local`:6379 --cluster-slave --cluster-master-id 47f7d58bff8f6c37691c1ee20e9024f21ce1ccf2
redis-cli  --cluster add-node `dig +short redis-4.redis.app-dev.svc.cluster.local`:6379 `dig +short redis-1.redis.app-dev.svc.cluster.local`:6379 --cluster-slave --cluster-master-id 47f7d58bff8f6c37691c1ee20e9024f21ce1ccf2
redis-cli  --cluster add-node `dig +short redis-5.redis.app-dev.svc.cluster.local`:6379 `dig +short redis-2.redis.app-dev.svc.cluster.local`:6379 --cluster-slave --cluster-master-id 47f7d58bff8f6c37691c1ee20e9024f21ce1ccf2
```
